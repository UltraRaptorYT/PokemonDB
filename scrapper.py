...
for pokemon in pokemonArr:
    ...
    # # Pokedex
    # pokeDexResult = soup.find_all(
    #     "a", attrs={"title": "List of Pokémon by National Pokédex number"})
    # pokeDex = int(pokeDexResult[1].get_text().replace("#", ""))

    # # Typings
    # typeResult = soup.find_all(
    #     "a", attrs={"title": re.compile(r"[A-Za-z]+ \(type\)$")})
    ...
    # # Abilities
    # hiddenAbilityHeader = soup.find(
    #     'small', text=re.compile(r"Hidden Ability"))
    # if hiddenAbilityHeader.parent.get("style") == "display: none":
    #     hiddenAbility = ""
    # else:
    #     hiddenAbility = hiddenAbilityHeader.parent.find_all("span")[
    #         0].get_text()
    # abilityArr = hiddenAbilityHeader.parent.parent.find_all(
    #     "td")[0].get_text().strip().split("\xa0or ")
    # if len(abilityArr) > 1:
    ...
    # # Height
    # heightHeader = soup.find(
    #     'td', text=re.compile(r"\d+(\.\d+)? m"))
    # height = float(heightHeader.get_text().replace(" m", ""))
    # if height.is_integer():
    #     height = int(height)

    # # Weight
    # weightHeader = soup.find(
    #     'td', text=re.compile(r"\d+(\.\d+)? kg"))
    ...
    # # Attack
    # AttackHeader = soup.find(
    #     'span', text="Attack")
    # Attack = int(AttackHeader.parent.parent.parent.find_all(
    #     "div")[1].get_text())

    # # Defense
    # DefenseHeader = soup.find(
    #     'span', text="Defense")
    # Defense = int(DefenseHeader.parent.parent.parent.find_all(
    #     "div")[1].get_text())

    # # Sp. Atk
    # SpAtkHeader = soup.find(
    #     'span', text="Sp. Atk")
    # SpAtk = int(SpAtkHeader.parent.parent.parent.find_all(
    #     "div")[1].get_text())

    # # Sp. Def
    # SpDefHeader = soup.find(
    #     'span', text="Sp. Def")
    # SpDef = int(SpDefHeader.parent.parent.parent.find_all(
    #     "div")[1].get_text())

    # # Speed
    # SpeedHeader = soup.find(
    #     'span', text="Speed")
    # Speed = int(SpeedHeader.parent.parent.parent.find_all(
    #     "div")[1].get_text())

    # # Total
    # TotalHeader = soup.find(
    #     'div', text=re.compile(r"Total"))
    # Total = int(TotalHeader.parent.find_all(
    #     "div")[1].get_text())

    # df.append([pokeDex, 1, int(str(pokeDex) + "1"), pokemon, type1,
    #           type2, "COLOR", ability1, ability2, hiddenAbility, 9, 0, 0, height, weight, HP, Attack, Defense, SpAtk, SpDef,Speed,Total])
    print(pokemon)
